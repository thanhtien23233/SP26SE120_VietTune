 ...
 *
 * CÁCH DÙNG:
 * 1. Copy file này vào project: src/components/BackgroundPatterns.jsx
 * 2. Import pattern bạn muốn:
 *
 *    import { ZitherStrings } from "@/components/BackgroundPatterns";
 *    // hoặc
 *    import { SoundWaves } from "@/components/BackgroundPatterns";
 *    // hoặc
 *    import { BrocadeDiamonds } from "@/components/BackgroundPatterns";
 *    // hoặc
 *    import { CloudScrolls } from "@/components/BackgroundPatterns";
 *    // hoặc
 *    import { BronzeDrum } from "@/components/BackgroundPatterns";
 *    // hoặc
 *    import { BambooGrid } from "@/components/BackgroundPatterns";
 *
 * 3. Đặt vào panel trái trang Login:
 *
 *    <div className="relative overflow-hidden ...class cũ...">
 *        <ZitherStrings />
 *        <div className="relative z-10">
 *            ...logo, tagline giữ nguyên...
 *        </div>
 *    </div>
 *
 * Muốn đổi pattern? Chỉ cần thay tên component, không cần sửa gì khác.
 */
...
export function SoundWaves() {
  const waves = Array.from({ length: 8 }, (_, i) => {
    ...
    return { d, op: 0.15 - i * 0.012, w: i % 3 === 0 ? 0.8 : 0.5 };
  });
      ...
      >
        <defs>
          <radialGradient id="sw-glow" cx="30%" cy="50%">
            <stop offset="0%" stopColor="#5a3a20" stopOpacity="0.35" />
        ...
        {waves.map((w, i) => (
          <path key={i} d={w.d} fill="none" stroke="#c9a84c" strokeWidth={w.w} opacity={w.op} />
        ))}
        ...
        {rings.map((ring, ri) =>
          ring.rs.map((r, ci) => (
            <circle
              ...
              strokeWidth="0.5"
              opacity={0.04 + ci * 0.02}
            />
          ))
        )}

        {[100, 220, 340, 460].map((x, i) => (
          <line key={i} x1={x} y1="0" x2={x} y2="600" stroke="#c9a84c" strokeWidth="0.2" opacity="0.04" />
        ))}
      </svg>
  ...
  return (
    <>
      ...
      <div
        className="absolute inset-0"
        style={{
          background: "linear-gradient(180deg, rgba(30,21,24,0.2) 0%, rgba(30,21,24,0.5) 50%, rgba(30,21,24,0.2) 100%)",
        }}
      />
    </>
  );
