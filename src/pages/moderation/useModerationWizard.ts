...
export function useModerationWizard(opts: {
    ...
    const [verificationForms, setVerificationForms] = useState<Record<string, ModerationVerificationData>>({});

    const primeClaimState = useCallback((submissionId: string, moderationData?: ModerationVerificationData) => {
    ...
    const nextVerificationStep = useCallback(
        ...
        [allItems, getCurrentVerificationStep, load, onRequireApproveConfirm, userId, validateStep, verificationForms],
    );

    const updateVerificationForm = useCallback((id: string | null, step: number, field: string, value: boolean | string) => {
        if (!id) return;
        setVerificationForms((prev) => {
